...
void Address(int pin,USER_DETAIL *acc,ADDRESS *temp)
{
	...
	if(temp->pincode==pin && addconfirm==1)
	{
		...
		switch (a)
		{
			...
			case 2:								// if user want to check another address with same pin code
				break;
			case 3:
				addconfirm=3;					// if user want to enter the address by themself 
				break;
			case 4:
				addconfirm=4;					// if user want to check for another pin code
				break;
		}
...
void Username(USER_DETAIL *acc,USER_DETAIL *roothead)
{
	...
	if(addconfirm)										// if username not found then confirm the username
	{
